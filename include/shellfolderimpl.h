...
public:
		...
		PCWSTR szDescription, PCWSTR szRootExt, UINT nFriendlyTypeNameId) throw()
	{
		...
		ATLVERIFY(SUCCEEDED(StringFromCLSID(T::GetObjectCLSID(), olestrCLSID)));
	...
	// Registration function to register as common shellfolder.
		...
		PCWSTR szDescription, PCWSTR szExtension, UINT nFriendlyTypeNameId) throw()
	{
		return UpdateRegistryFromResource(nResId, bRegister,
			szDescription, T::GetObjectCLSID(), szExtension, nFriendlyTypeNameId);
	}
	...
	static HRESULT WINAPI UpdateRegistry(UINT nResIdRoot, UINT nResIdExt, BOOL bRegister,
		PCWSTR szDescription, PCWSTR szRootExt, PCWSTR szExtension, UINT nFriendlyTypeNameId) throw()
	{
		// note: during unregistration try to unregister as much as possible.
		HRESULT hrFinal = S_OK;

		HRESULT hr = UpdateRegistryForRootExt(nResIdRoot, bRegister, szDescription,
		                                      szRootExt, nFriendlyTypeNameId);
		if (FAILED(hr))
			hrFinal = hr; 
	...
	static HRESULT WINAPI UpdateRegistry(UINT nResIdStd, UINT nResIdWin98, BOOL bRegister,
		PCWSTR szDescription, PCWSTR szExtension, UINT nFriendlyTypeNameId) throw()
	{
		UINT nResId = IsShell5OrHigher() ? nResIdStd : nResIdWin98;
		return UpdateRegistry(nResId, bRegister, szDescription, szExtension, nFriendlyTypeNameId);
	}
